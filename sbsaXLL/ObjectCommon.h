...
#define SBSA_XLL_CATCH_OPER() \
   ...
   {\
        ObjectHandler::RepositoryXL::instance().logError(sbsaObjects::error, functionCall);\
        ObjectHandler::scalarToOper(sbsaObjects::error, returnValue);\
        return &returnValue;\
    };



#define SBSA_XLL_CATCH_STRING() \
   catch (const std::exception &e) \
   {\
        ObjectHandler::RepositoryXL::instance().logError(e.what(), functionCall);\
    ...
    {\
        ObjectHandler::RepositoryXL::instance().logError(sbsaObjects::error, functionCall);\
        ObjectHandler::stringToChar(sbsaObjects::error, ret);\
        return ret;\
    };
}
